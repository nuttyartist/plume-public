...
{
    if (m_blockModel != nullptr) {
        m_blockModel->setCurrentNodeType(static_cast<NodeItem::Type>(index.data(NodeItem::Roles::ItemType).toInt()));
    }
    setCurrentIndex(index);
    clearSelection();
    setSelectionMode(QAbstractItemView::SingleSelection);
...
void NodeTreeView::setBlockModel(BlockModel* newBlockModel)
{
    m_blockModel = newBlockModel;
}
